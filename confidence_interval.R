...
h_list = 1:66
list_origin_date = seq_quotation_date(ymd("2023-05-18"), 69 - 10) # 68 because the 68th day is the last day of available data when I am writing the script
...
for(i_date in seq_along(list_origin_date)){
  x = readRDS(paste0("../data_daily_forecast/",list_origin_date[i_date],"_forecast.rds"))
...
sorted_ratio_array <- apply(ratio_array, c(1,2), sort)
...
#quantile_array = array(0, dim = c(2, length(models),length(h_list_ic)))

get_quantiles <- function(vector){
  return(c(vector[[n_lower_quantile]],vector[[n_upper_quantile]]))
}

quantile_array <- apply(sorted_ratio_array, c(2,3), get_quantiles)

saveRDS(quantile_array, file = "../data_daily_forecast/quantile_array.rds")
